...
namespace veritabaniuyg
{
    public partial class playlist : Form
    {
        ...
        {   //ekle
            baglanti.Open();
            NpgsqlCommand komut1 = new NpgsqlCommand("insert into playlist (playlistid,playlistadi,sarkiid,kullaniciid,kacsarkivar) values (@p1,@p2,@p3,@p4,@p5)", baglanti);
        ...
        {   //sil
            baglanti.Open();
            NpgsqlCommand komut2 = new NpgsqlCommand("Delete from playlist where playlistid=@p1", baglanti);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            //guncelle
            baglanti.Open();
            NpgsqlCommand komut3 = new NpgsqlCommand("update playlist set playlistadi=@p1,sarkiid=@p2,kullaniciid=@p3,kacsarkivar=@p4 where playlistid=@p5", baglanti);
        ...
        {             // Listeleme işlemi
            string sorgu = "select * from playlist";
            ...
            try
            {
                using (var conn = new NpgsqlConnection("server=localHost; port=5432; Database=sarkilar; user Id=postgres; password=12345"))
                {
                    ...
                    using (var cmd = new NpgsqlCommand("SELECT birsarkikacfarklilistede(@sarki_id)", conn))
                    {
                        cmd.Parameters.AddWithValue("@sarki_id", int.Parse(textBox3.Text));
